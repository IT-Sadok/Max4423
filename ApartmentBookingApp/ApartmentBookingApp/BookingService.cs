namespace ApartmentBookingApp
{
    internal class BookingService
    {
        ...
        public void AddHost()
        {
            ...
            _hosts.Add(new Host(){Id = _idGeneratorService.GetNextHostId(),FullName = fullName, PhoneNumber = phoneNumber});
        }
        ...
        public void ShowHosts()
        {
            ...
            {
                var host = _hosts[i];
                Console.WriteLine($"[{i + 1}]. Id: {host.Id} | FullName: {host.FullName} | PhoneNumber: {host.PhoneNumber}");
            }
        }
        ...
        public void AddApartmentToHost()
        {
            ...
            _hosts[hostNumber - 1].Apartments.Add(new Apartment(){Id = _idGeneratorService.GetNextApartamentId(),Title=title,PricePerNight=pricePerNight,Capacity=capacity});
        }

        
        public void ShowApartmentByHostId()
        {
            if (_hosts.Count == 0)
            {
            ...
            var hostNumber = InputHelper.ReadIntValue(
            ...
            {
                Console.WriteLine(
                    $"{i + 1}. Id: {_hosts[hostNumber - 1].Apartments[i].Id} | Title: {_hosts[hostNumber - 1].Apartments[i].Title} | PricePerNight: {_hosts[hostNumber - 1].Apartments[i].PricePerNight} | Capacity: {_hosts[hostNumber - 1].Apartments[i].Capacity}");
            }
        }
